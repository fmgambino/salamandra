...
function boot(){initData();migrateSensorUnits();ensureActuatorEvents();setupForms();setupUi();applyTheme();renderAll();logoLight.value=state.settings.logoLight;logoDark.value=state.settings.logoDark;faviconUrl.value=state.settings.favicon;installIcon.value=state.settings.installIcon;startRealtimeSimulator();if(state.user)loginOK(state.user)}boot();
